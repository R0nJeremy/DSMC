...
class DSMC_Mono:
    def __init__(self, _N=1_000_00) -> None:
        #* simulation space constants
        self.N_particles = _N # number of particles in the system
        self.n = 0.02 # number density (1/meter^3)
        ...
        self.L_box = np.power(self.V/(self.Nx*self.Ny*self.Nz),1/3) # linear size of a box (meter)
        ...
        self.rho = 900 # mass density of a particle. The value is for water-ice (kg/meter^3)
        ...
        self.box_particles = np.ndarray((self.Nx, self.Ny, self.Nz), dtype=object) # list of indices of particles in a box
        self.box_vmax = np.ndarray((self.Nx, self.Ny, self.Nz), dtype=np.float64) # vmax per box
        self.box_nc_error = np.ndarray((self.Nx, self.Ny, self.Nz), dtype=np.float64) # rounding error for number of collisions per box
    ...
    def _init(self) -> None:        
        ...
        print(f"Linear sizes of the system:          Lx = {self.Lx:.2f} (m), Ly = {self.Ly:.2f} (m), Lz = {self.Lz:.2f} (m)")
        print(f"Volume of the simulated space:       V = {self.V:_.2f} (m^3)")
        print(f"Number of boxes:                     Nx = {self.Nx}, Ny = {self.Ny}, Nz = {self.Nz}")
        print(f"Linear size of a box:                L_box = {self.L_box:.2f} (m)")
        print(f"Volume of a box:                     V_box = {self.V_box:_.2f} (m^3)")
        print(f"Total number of boxes:               N_total = {self.Nx * self.Ny * self.Nz:_}")
        print(f"Average number of particles per box: Nbox = {self.N_particles/(self.Nx*self.Ny*self.Nz):.2f}")
        ...
        self.vx = np.random.default_rng().normal(0, np.sqrt(self.T0/self.m),self.N_particles)
        self.vy = np.random.default_rng().normal(0, np.sqrt(self.T0/self.m),self.N_particles)
        self.vz = np.random.default_rng().normal(0, np.sqrt(self.T0/self.m),self.N_particles)
        ...
        print("...")
        print(f"Initial temperature:                 {self.T0:.2f} (J)")
        print(f"Sampled temperature:                 {self.temperature:.2f} (J)")
        print(f"Mass density of the system:          {self.m*self.N_particles/self.V:.5f} (kg/m^3)")
        print(f"Pressure:                            {self.n*self.temperature:.5f} (Pa)")
        print(f"Characteristics time:                {self.nu_c:.5f} (1/s)")
        print("...")
    ...
    def set_system_state(self) -> None:        
        self.temperature = (self.m/3)*(self.vx**2 + self.vy**2 + self.vz**2).mean()
        ...
        for box_idx, box_list in np.ndenumerate(self.box_particles):
            if box_list.size == 0:
                ...
                temperature_box = (self.m/3)*(self.vx[box_list]**2 + 
                                              self.vy[box_list]**2 + 
                                              self.vz[box_list]**2).mean()
    ...
    def collisions(self) -> None:
        ...
        for box_idx, box_list in np.ndenumerate(self.box_particles):
            ...
            if n_box_particles > 1:
                Nc_float = self.C_ncoll*n_box_particles*(n_box_particles-1)*\
                            self.box_vmax[box_idx]*self.dt + self.box_nc_error[box_idx]
                Nc = int(Nc_float)
                self.box_nc_error[box_idx] = Nc_float - Nc
                for coll_idx in range(Nc):
                    p_index_1, p_index_2 = np.random.default_rng().choice(box_list, 2, replace=False)
                    n_collisions += self.binary_collision(p_index_1, p_index_2, self.box_vmax[box_idx])
    ...
    def haff_cooling(self, t: float) -> float:
        tc_inv = (8/3)*(1-self.eps**2)*self.n*self.R**2*np.sqrt(np.pi*self.T0/self.m)
...
if __name__ == "__main__":
    ...
    ax1.plot(time_array, Temp_array, time_array, sim.haff_cooling(time_array))
    ax1.set_xlabel("Time (s)")
    ax1.set_ylabel("Granular temperature (J)")
    ax1.set_xscale("log")
    ax1.set_yscale("log")
    
    ax2.plot(time_array, NcolCum_array, time_array, sim.ncol_analytic(time_array))
    ax2.set_xlabel("Time (s)")
    ax2.set_ylabel("Cumulative number of collisions")
    plt.show()
